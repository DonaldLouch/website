...
export default function EducationExperience({ education, resumeID }: any) {
  ...
  const deleteEducation = async () => {
    ...
    // })
    // deleteStatus === 204 && router.invalidate()
  }
  ...
  return <>
    <PrimaryButton key={education.id} onClick={open} icon={{ name: "graduation-cap" }}>{education.school}</PrimaryButton>
    <Modal
    ...
    >
        <Box p="0 2rem 0" component="form" onSubmit={form.onSubmit(onSubmit)}>
          ...
          <SimpleGrid cols={2} spacing="2rem" style={{ alignItems: "center" }}>
            <PrimaryButton onClick={deleteEducation} icon={{ name: "trash" }} bg="red">Delete Education Experience</PrimaryButton>
