...
type VideoPlayerProps = {
    video: any
    description: any
    playerType: "page"|"embed"
    isUser: boolean
    isAdmin: boolean
}

export default function VideoPlayer({ video, description, playerType, isUser, isAdmin }: VideoPlayerProps) {
    const theme = useMantineTheme();
    const mobile = useMediaQuery(`(max-width: ${theme.breakpoints.sm})`);

    const { VideographyMedia: media, ThumbnailMedia: thumbnail } = video;
    ...
    return (<>
        <Box
            ...
            mb="-md"
        >
            {!isPublic && !isUser && !isAdmin ? <Paper p="10rem 2rem" color="white" bg="none" shadow="bsBoldSecondary" radius="lg">
                <Stack align="center">
                ...
                <Text>🚨 This video is listed as a private video and is not viewable
                  to the public. 🚨</Text>
                  <PrimaryLinkedButton link={{ to: "/feed/videography" }} icon={{ name: "arrow-left", pack: "fadl" }}>Go Back To The Video Feed</PrimaryLinkedButton>
                </Stack>
            </Paper> : <>
                <Stack
                    style={{ 
                ...
                {!isEmbed && isHorizontal &&
                    <Box p={{base: "1rem", sm: "1rem 5rem"}}>
                        <FullDescription video={video} description={description} />
                    </Box>
                }
                ...
                >
                    <Text fw="600">Share via Link</Text>
                        <CopyButton value={`${process.env.VITE_SITE_URL}/video/${video.id}`}>
                            {({ copied, copy }) => (
                                <Group style={{boxShadow: copied ? "var(--mantine-shadow-bsBoldPrimary)" : "var(--mantine-shadow-bsBoldRed)", borderRadius: "var(--mantine-radius-md)"}} p="0.5rem 2rem" onClick={copy}>
                                    ...
                                    <Text>{copied ? `Link Copied: ${process.env.VITE_SITE_URL}/video/${video.id}` :`${process.env.VITE_SITE_URL}/video/${video.id}`} </Text>
                                </Group>
                            )}
                        </CopyButton>
