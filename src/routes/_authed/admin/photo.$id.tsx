...
import { Box, Text, Image, ActionIcon, Anchor, Badge, Flex, Group, Stack, Title } from '@mantine/core'
import { GetMarkdown } from '@/actions/markdown.functions'
import { GetAllLocationData, GetAllPhotographyAlbums, GetAllTagData, GetPhoto } from '@/actions/database/GetDatabase.functions'
...
export const Route = createFileRoute('/_authed/admin/photo/$id')({
    ...
    loader:  async ({ params }) => {
        ...
        return { 
            photo: await GetPhoto({ data: {id} }) as any,
            photographyAlbum: await GetAllPhotographyAlbums(),
            locations: await GetAllLocationData(),
            tagsData: await GetAllTagData(),
    ...
    // head: () => ({
        meta: [
            ...seo({
                title: `Edit Photo: ${loaderData?.photo?.id} | ${import.meta.env.VITE_WEBSITE_NAME}`,
            }),
        ]
    })
})
...
function RouteComponent() {
    // const { isUser, isAdmin, photo, photographyAlbum, locations, tagsData } = Route.useLoaderData()
    // const navigate = useNavigate()
    // const router = useRouter()
    // !isAdmin && navigate({ to: "/auth", search: { message: "NoAccess" } })

    // const {fileID, fileTitle, filePath, capturedOn, uploadedOn, fileKey, fileVersionID } = photo.PhotographyMedia
    // const { photoName, caption, tags, links, album, location, id: photoID, isPublic, isPortfolio, isPinned } = photo
    ...
    //     })
    //     supabaseStatus === 204 && router.invalidate()
