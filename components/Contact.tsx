...
export default function Contact() {
  ...
  async function sendContact(contactData: any) {
    ...
    const response = await fetch("/api/message/newContact", {
      method: "POST",
      body: JSON.stringify(contactData),
    });
    ...
    if (response.status === 500) {
      toast({
        ...
        description:
          "It seems like an error occurred while trying to submit your contact form to Donald Louch. Please try again.",
        status: "error",
        duration: 9000,
        isClosable: true,
      });
