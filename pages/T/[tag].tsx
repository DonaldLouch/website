  ...
  return (
    ...
    <Metadata
        ...
        <Box as="main" color={useColorModeValue("black", "white")}>
          ...
          <SectionTitle headingTitle={posts.length === 0 ? (`It seems that there is no blog posts under the tag "${router.query.tag}".`) : (`${router.query.tag === "design" ? ("Graphic Design") : (`"${router.query.tag}"`)} has ${posts.length} ${posts.length === 1 ? ("post") : ("posts")} in it's tag`)} />
