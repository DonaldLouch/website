...
export default function TermsOfServicePortal({ aboutData }:any) {
  ...
  async function updateAbout(updateAboutData: any) {
    ...
    return (
        <>  
            <PortalLayout pageTitle={`Edit: Terms of Service Page`}>
                <Metadata
                    title={`Edit: Terms of Service | ${process.env.WEBSITE_NAME}`}
                    keywords={`${process.env.KEYWORDS}, portal, edit, admin`}
                    description={`Edit the Terms of Service page on the ${process.env.WEBSITE_NAME} portal.`}
                    />
                    <Box as="main" id="editTerms" color="black">
