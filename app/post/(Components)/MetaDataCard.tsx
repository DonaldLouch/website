...
export const MetaDataCard = (post: any) => {
    ...
    return (
        <Box bg="backgroundGradient" p="0.5rem 1.5rem" my="1.5rem" color="white" borderRadius="0 2rem">
            <Text fontSize="1rem" color="white">By: <Link href="/about">{post.author}</Link> | Posted On: {postedOn} | Filed Under: {categorySplit.map((category: any) => (<Link key={category} href={`/C/${category}`} variant="unstyled" textDecoration="none" color="currentColor" boxShadow="bsBoldWhite" p="0.5rem" borderRadius="0 0.5rem" mx="0.2rem" _hover={{boxShadow: "bsBoldSecondary"}}>{category}</Link>))}</Text>
        </Box>
    )
}