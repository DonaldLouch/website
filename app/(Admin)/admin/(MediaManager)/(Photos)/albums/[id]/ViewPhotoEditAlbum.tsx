...
export default function ViewPhotoEditAlbum({ locations, photoData }: any) {
    ...
    return (<>
        <Box onClick={onOpen} _hover={{background: "blurredBackground", opacity: "0.6"}} transition="all 0.1s" display="inline-block">
            <Image src={photoData.fileID.filePath} alt={`${photoData.fileID.fileID}-${photoData.fileID.fileTitle}`} width="500" height="500" quality={20}/>
        </Box>
        <Modal isOpen={isOpen} onClose={onClose} id={`viewPhoto${photoData.id}`} size="7xl" isCentered>
            <ModalOverlay
                bg='blackAlpha.300'
                backdropFilter='blur(10px) hue-rotate(90deg)'
            />
            <ModalContent background="blurredPurple" mx="2rem" pb="2rem">
                <ModalHeader>{photoData.photoName} {photoData.id}</ModalHeader>
                <ModalCloseButton />
                <ModalBody>
