...
export default function VideoUploader({currentStep, categoryData}: {currentStep: number, categoryData:any}) {
    ...
    const breadCrumbs = [
        ...
        {"pageLink": "/portal/videography/upload", "pageName": `Videography Upload: ${videoID}`},
    ]
    ...
    async function startUploader() {
        ...
        status === 201 && router.push('/portal/videography/upload?step=2')
    }
    
    async function refreshUploader() {
        destroyCookie({}, 'videoID', {path: '/'})
        router.push('/portal/videography/upload?step=1')
