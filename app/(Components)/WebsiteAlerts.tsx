...
export default function WebsiteAlerts() {
    ...
    return <>
        ...
        <Modal opened={opened} onClose={close} title="Website Notifications" yOffset="5rem" xOffset="5rem" size="100%"  
            ...
            : alertData?.length === 0 || !alertData ? <Alert variant="light" color="green" title="No Alerts!" icon={<CheckmarkBadge03Icon variant="twotone"/>}><Text my="0.5rem" c="white">There are no alerts for the website at this time!</Text></Alert> :
                <Tabs defaultValue="all" style={{boxShadow: "none"}} mt="0">
                    ...
                    <Tabs.Panel value="active">
                        {!loading && activeAlerts.length === 0 ? <Alert variant="light" color="green" title="No Alerts!" icon={<CheckmarkBadge03Icon variant="twotone"/>}><Text my="0.5rem" c="white">There are no active alerts for the website at this time!</Text></Alert> : alertData && activeAlerts?.map((alert: any) => {
                            const isActive = !alert.EndedOn || alert.EndedOn && new Date(alert.EndedOn && alert.EndedOn) >= new Date()
                            const icon = alert.AlertIcon === "Cone01Icon" ? <Cone01Icon variant={alert.AlertIconStyle ? alert.AlertIconStyle : "stroke"} /> 
                            : <AlertCircleIcon variant={alert.AlertIconStyle ? alert.AlertIconStyle : "stroke"} />
                    ...
                    <Tabs.Panel value="ended">
                        {!loading && endedAlerts.length === 0 ? <Alert variant="light" color="green" title="No Alerts!" icon={<CheckmarkBadge03Icon variant="twotone"/>}><Text my="0.5rem" c="white">There are no alerts that have ended at this time!</Text></Alert> : alertData && endedAlerts.map((alert: any) => (<Box key={alert.id} p="2rem" m="1rem 2rem" style={{boxShadow: "var(--mantine-shadow-bsBoldPrimary)", borderRadius: "var(--mantine-radius-md)"}}>
                                <Stack style={{borderBottom: "1px solid"}} gap="0">
                                    <Title order={3} fz="1.5rem">{alert.AlertTitle}</Title>
                                    <Text my="0" size="sm" c="gray" pb="0.5rem">Created on <strong><DisplayDate source={alert.CreatedOn} /></strong> and ended on <strong><DisplayDate source={alert.EndedOn} /></strong></Text>  
                                </Stack>
