...
export default function FormInputPhone( props: any) {
    const { inputID, inputLabel, inputDescription, isRequired, icon, formProps, isFieldDisabled, ...rest } = props
        ...
        <Grid gutter="1rem">
            ...
            <Grid.Col span={inputDescription ? {base: 12, sm: 9} : 12}>
                <Input.Wrapper
                    ...
                    {...rest}
                >
                    <Input styles={{input: {padding: !icon ? "1rem 1.5rem" : "1rem 4rem", margin: "0.5rem -0.5rem"}, section: {marginLeft: "-0.5rem", borderRight: "solid 0.2px grey"}}} classNames={{input: classes.defaultInput}} leftSection={icon && icon} name={inputID} type="tel" 
                    // pattern="\\([0-9]{3}\\)\\s[0-9]{3}-[0-9]{4}"
                     disabled={isFieldDisabled} c={isFieldDisabled ? "primary" : "white"} w="100%" {...rest} />
                </Input.Wrapper>
            </Grid.Col>
         </Grid>
    )
