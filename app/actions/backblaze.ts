    ...
    const response = await Promise.all(
      files.map(async (file) => {
        ...
        const isFileUploaded =
          upload.$metadata.httpStatusCode === 200 ? true : false;

        if (isFileUploaded) {
          try {
            const id = mediaID
              ? mediaID
              : uploadDestination === "photography"
                ? "photo" +
                  Date.now().toString(36).toUpperCase() +
                  Math.random().toString(36).substring(2, 5).toLowerCase()
                : "file" +
            ...
            const { status: databaseS1Status, error: databaseS1Error } =
              await supabase
                .from(
                  uploadDestination === "photography"
                    ? "PhotographyMedia"
                    : uploadDestination === "videography"
                      ? "VideographyMedia"
                      : uploadDestination === "thumbnail"
                        ? "ThumbnailMedia"
                        : null,
                )
                .insert({
            ...
            if (databaseS1Status === 201) {
              try {
                ...
                throw new Error(
                  "Couldn't upload the file to the database (2).",
                );
              }
            }
            return {
              ...
              fileSetup:
                isFileUploaded && databaseS1Status === 201 ? true : false,
              fileID,
              id,
              fileName,
              filePath,
...
export async function deleteFileFromS3({
  ...
  filePath,
  getFileID,
}: S3Payload): Promise<DeleteObjectCommandOutput> {
  ...
  try {
    ...
    const isFileDeleted =
      sendDelete.$metadata.httpStatusCode === 204 ? true : false;
    try {
      isFileDeleted &&
        (await supabase
