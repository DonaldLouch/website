    ...
    const onSubmit = async (values: any) => {
        ...
        const { status: supabaseStatus , error: supabaseError } = await supabase.from("Tasks").insert({ 
         ...
         supabaseStatus && notifications.show({ 
            ...
            icon: supabaseStatus === 201 ? <HugeIcon name="task-add-01" /> : <HugeIcon name="alert-diamond" />
        })
                ...
                <Box component="form" onSubmit={form.onSubmit(onSubmit)} m="-1rem 2rem 2rem">
                    <Grid w="100%" justify="center" align="center">
                        <Grid.Col span={10} pl="2rem">
                            <FormInput inputID="title" {...form.getInputProps('title')} icon={<HugeIcon name="text-font" variant="twotone" />} w="100%" />
                        </Grid.Col>
                        <Grid.Col span={2}>
                            <FormSubmitButton icon={<HugeIcon name="task-add-01" />} isNotFull={true}>Add Task</FormSubmitButton>
                        </Grid.Col>
                    </Grid>
                </Box>
            }
            {tasks.length === 0 ? <Alert variant="light" color="green" icon={<HugeIcon name="information-circle" variant="twotone"/>} mt="3rem"><Text my="0.5rem" c="white">There currently is no tasks assigned for this project. Donald Louch will add new tasks as the project progresses so make sure to check back!</Text></Alert> :
